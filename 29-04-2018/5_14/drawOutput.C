...
//  BNegPeak->SetParameters(&FinPar_neg[5]);
  ...
  MassBPos->SetTitle("Local Asymmetry B^{+} #rightarrow K^{+} #pi^{-} #pi^{+}");
 ...
 //Draw Legend:
  ...
  MassBNeg->SetTitle("Local Asymmetry B^{-} #rightarrow K^{-} #pi^{-} #pi^{+}");
  ExpBG_Neg->SetParameters(&FinPar_neg[0]);
  FourBodyBG_Neg->SetParameters(&FinPar_neg[2]);
  BNegPeak->SetParameters(&FinPar_neg[5]);
...
/*
  ...
  */
  ...
  Asym_Sig_Local_Dalitz->SetMaximum(10);
  Asym_Sig_Local_Dalitz->SetMinimum(-10);
